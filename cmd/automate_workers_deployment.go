...
// automateWorkersDeploymentCmd represents the automate_workers_deployment command
var automateWorkersDeploymentCmd = &cobra.Command{
	Use:              "automate_workers_deployment siteName [--build_path=<local_build_path>]",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// and automate_masters_deployment, so the site directory and required
		// automation configs are already available on disk
...
func init() {
	rootCmd.AddCommand(automateWorkersDeploymentCmd)

	automateWorkersDeploymentCmd.Flags().StringP("build_path", "", "", "Directory to use as build path. If that doesn't exist, the installer will generate a default directory")
}
