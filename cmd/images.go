...
func GenerateDeploymentImage(buildPath string, releasesURL string, version string) {
	...
	gzipfile, err := os.Open(localQcow2)

	if err != nil {
		log.Fatal(fmt.Sprintf("Error opening qcow2 file: %s", err))
		os.Exit(1)
	}

	reader, err := gzip.NewReader(gzipfile)
	if err != nil {
		log.Fatal(fmt.Sprintf("Error reading compressed file: %s", err))
		os.Exit(1)
	}
	defer reader.Close()

	newfilename := fmt.Sprintf("%s/rchos-qemu.qcow2", buildPath)
	writer, err := os.Create(newfilename)

	if err != nil {
		log.Fatal(fmt.Sprintf("Error creating uncompressed file: %s", err))
		os.Exit(1)
	}
	...
	if _, err = io.Copy(writer, reader); err != nil {
		log.Fatal(fmt.Sprintf("Error generating uncompressed file: %s", err))
		os.Exit(1)
	}
}
