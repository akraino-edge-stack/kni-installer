...
func init() {
	...
	prepareManifestsCmd.Flags().StringP("site", "", "", "Url/path for site repository. Can be in any go-getter compatible format")
	prepareManifestsCmd.MarkFlagRequired("site")
	prepareManifestsCmd.Flags().StringP("build_path", "", "", "Directory to use as build path. If that not exists, the installer will generate a default directory")
