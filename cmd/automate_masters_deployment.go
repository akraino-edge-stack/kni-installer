...
// automateMastersDeploymentCmd represents the automate_masters_deployment command
var automateMastersDeploymentCmd = &cobra.Command{
	Use:              "automate_masters_deployment siteName [--build_path=<local_build_path>]",
...
func init() {
	rootCmd.AddCommand(automateMastersDeploymentCmd)

	automateMastersDeploymentCmd.Flags().StringP("build_path", "", "", "Directory to use as build path. If that doesn't exist, the installer will generate a default directory")
}
