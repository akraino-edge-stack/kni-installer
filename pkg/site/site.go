...
func (s Site) FetchRequirements() {
	...
	profileFile := fmt.Sprintf("%s/00_install-config/kustomization.yaml", siteBuildPath)
	...
	if _, err := os.Stat(profileFile); err == nil {
		...
		for scanner.Scan() {
			...
			r := requirements.New(strings.TrimSpace(requirementsBits[0]), strings.TrimSpace(requirementsBits[1]), siteBuildPath)
			r.FetchRequirement()
		}
